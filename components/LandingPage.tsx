...
import { BookOpen, Sparkles, Feather, Globe, Star } from 'lucide-react';
...
export const LandingPage: React.FC<LandingPageProps> = ({ onStart, galleryBooks, onViewBook }) => {
  return (
                    ...
                    <div className="text-center w-full px-2">
                        ...
                        <div className="flex flex-wrap justify-center gap-1 mb-3 opacity-60 group-hover:opacity-100 transition-opacity">
                            {book.prefs.interests.slice(0, 2).map(i => (
                                <span key={i} className="text-[10px] uppercase tracking-wider px-2 py-0.5 bg-slate-100 text-slate-600 rounded-sm">
                                    {i}
                                </span>
                            ))}
                        </div>
